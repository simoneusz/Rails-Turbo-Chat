...
RSpec.describe Api::V1::Queries::RoomsQuery do
  describe '.filter_definition' do
    ...
    it 'includes allowed filters for name' do
      expect(described_class.filters_definition['name']).to match_array(%w[eq])
    end
  ...
  describe '.sort_definition' do
    it 'includes allowed sorts' do
      expect(described_class.sorts_definition.map do |array|
        array[0]
      end).to match_array(%w[id name created_at updated_at])
    end
  end
end
