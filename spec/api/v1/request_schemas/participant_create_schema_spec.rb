...
RSpec.describe Api::V1::RequestSchemas::ParticipantCreateSchema do
  describe '#call' do
    ...
    context 'with invalid params' do
      context 'when params have invalid type' do
        ...
        it 'returns an error message for name' do
          expect(schema.errors[:user_id].join).to include(error_message)
        end
      end
    end
    ...
    context 'with missing required params' do
      ...
      it 'returns an error message' do
        expect(schema.errors[:user_id].join).to include('is missing')
      end
    end
  end
end
