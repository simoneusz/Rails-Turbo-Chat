...
RSpec.describe Api::V1::RequestSchemas::ContactSchema do
  describe '#call' do
    subject(:schema) { described_class.new.call(params) }

    context 'with valid params' do
      let(:params) { { contact_id: 1 } }
    ...
    context 'with invalid params' do
      context 'when params have invalid type' do
        ...
        it 'returns an error message for name' do
          expect(schema.errors[:contact_id].join).to include(error_message)
        end
      end
    end
    ...
    context 'with missing required params' do
      ...
      it 'returns an error message' do
        expect(schema.errors[:contact_id].join).to include('is missing')
      end
    end
  end
end
